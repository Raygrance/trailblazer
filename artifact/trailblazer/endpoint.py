from reqobj import RequestObject
import path
import psycopg2
import json
import model
import pickle
import constants


def fetch_requests(host):
    conn = psycopg2.connect(database=constants.DB_NAME, host=constants.DB_HOST, user=constants.DB_USER, password=constants.DB_PASS, port=constants.DB_PORT)
    cursor = conn.cursor()
        
    cursor.execute(f"SELECT * FROM requests WHERE host='{host}';")
    rows = cursor.fetchall()

    requests = []
    prev = None

    f = open("requests.txt", "x")

    for row in rows:
        #r_id, r_host, r_url, r_method, r_query, r_req, r_res, r_code, _ = row
        requests.append(row)

        with open("requests.txt", "a") as f:
            f.write(f"{str(row[5])}\n")

    return requests

def rule_to_parameters(rule):
    result = []
    i = 1
    for segment in rule:
        if type(segment) == int:
            parameter = model.OpenAPI_Parameter("id" + str(i), "path", True, model.OpenAPI_Schema("integer" if segment == 1 else "string"))
            result.append(parameter)
            i += 1
    return result

def to_spec(base_url, requests, examples=0):
    filename = (base_url + ":")[:base_url.find(":")]  # this removes the port number from the hostname
    ReqObjs = {}
    for request in requests:
        method = request[3].lower()
        rule = path.url_to_rule(request[2])

        if (rule, method) not in ReqObjs:
            ReqObjs[(rule, method)] = [RequestObject(), set()]
            #print(json.loads(request[5]))
        obj = json.loads(request[5])
        seed_id = request[0] # request[0] is the unique requestID
        ReqObjs[(rule, method)][0].parse(obj, seed_id) # add seed id alongside object data
        ReqObjs[(rule, method)][0].add_example(obj)
        ReqObjs[(rule, method)][1].add(request[7])

    with open("tests/" + filename + ".pkl", "wb") as file:
        pickle.dump(ReqObjs, file)

    result = model.OpenAPISpec(base_url)
    paths = model.OpenAPI_Paths()
    result.set_paths(paths)

    for rule, method in ReqObjs:
        rule_str = path.rule_to_path(rule)
        if rule_str not in paths:
            paths.add_pathItem(rule_str, model.OpenAPI_PathItem())
            for parameter in rule_to_parameters(rule):
                paths[rule_str].add_parameter(parameter)
        if method not in paths[rule_str].methods:
            paths[rule_str].methods[method] = model.OpenAPI_Operation()
        if ReqObjs[(rule, method)][0]:
            paths[rule_str].methods[method].set_requestBody(ReqObjs[(rule, method)][0].to_requestbody(examples=examples))
        for response_code in ReqObjs[(rule, method)][1]:
            paths[rule_str].methods[method].add_response(response_code, {"description": "Auto generated response object"})
    
    return result

def fetch_hosts():
    conn = psycopg2.connect(database=constants.DB_NAME, host=constants.DB_HOST, user=constants.DB_USER, password=constants.DB_PASS, port=constants.DB_PORT)
    cursor = conn.cursor()
    
    cursor.execute(f"SELECT DISTINCT host FROM requests;")
    rows = cursor.fetchall()
    
    result = []
    for row in rows:
        result.append(row[0])
    
    result.sort()
    i = 0
    for r in result:
        i += 1
        print(str(i) + "\t" + r)
    
    return result


def generate_spec_from_traffic(examples=5):
    hosts = fetch_hosts() # generates and prints a list of hosts
    print()
    print("Select a web application host from the above list: ", end="")
    s = int(input()) - 1 # user selects a host
    target = hosts[s] # target is set to the selected host
    filename = (target + ":")[:target.find(":")]  # this removes the port number from the hostname

    requests = fetch_requests(target) # fetches requests for the selected host

    spec = to_spec(target, requests, examples=examples) # generates a spec from the requests

    oas = spec.to_dict()

    f = open("tests/" + filename + ".json", "w")
    json.dump(oas, f, indent=4)
    f.close()

    endpoint_count = 0
    f = open("tests/" + filename + "_endpoints.txt", "w", encoding="utf-8")
    for path in oas["paths"]:
        for method in oas["paths"][path]:
            f.write(method + " " + path + "\n")
            endpoint_count += 1 #print(method, path)
    f.close()

    print()
    print(str(endpoint_count) + " endpoints identified for " + target + ". OpenAPI specification saved to tests/" + filename + ".json")


# if __name__ == "__main__":
#     hosts = path.step1()
#     print()
#     print("Select a web app from the above list: ")
#     s = int(input()) - 1

#     rules = path.step2(hosts[s])

#     # path.print_rules(rules)

#     print("====================================")

#     requests = fetch_requests(hosts[s])

#     #model = process(requests, rules)
#     #print_model(model)

#     spec = to_spec(hosts[s], requests, rules, examples=5, mutants=0)

#     temp = spec.to_dict()

#     #print(temp)

#     f = open("to_spec_test.json", "w")
#     json.dump(temp, f, indent=4)
#     f.close()


