...
from baseline import fetch_requests
...
def run_fuzz(target, n, m, r, g, headers=None):
    '''
    target: str
    ...
    r: bool
        Replay captured traffic from host by generating payloads based on ReqObjs
    g: bool
        Replay exact captured traffic from host 
    ...
    if r:
        ...
        for (rule, method), reqobj_list in ReqObjs.items():
            ...
            for _ in range(max(1, len(reqobj.examples))): # num captured requests for this endpoint
                ...
                payload = reqobj.generate(seed_id=seed_id) # generate a payload according to ReqObj 
                
                # send request
                try:
                    response = send_request(base_url=BASE_URL, method=method, path=path_str, body=payload,headers=headers_dict)
...
def normalise_db_request(row):
    """
    DB row:
    (id, host, url, method, query, req_body, res, code, timestamp)
    """
    _, host, url, method, query, req_body, _, _, _ = row

    path = url                      # already path-only
    body = None
    ...
    if req_body:
        try:
            ...
            body = None  # skip non-JSON bodies
...
def find_reqobj_and_schema(db_req, ReqObjs, schema_dict):
    for (rule, method), reqobj_list in ReqObjs.items():
        if method != db_req["method"]:
            continue

        rule_path = path.rule_to_path(rule)
        if rule_path != db_req["path"]:
            continue
